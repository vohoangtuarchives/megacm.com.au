...
class Func
{
    ...
    function __construct($d = null, $cache = null)
    {
        $this->d = $d;
        $this->cache = $cache;
    }
    ...
    public static function createThumb($width_thumb = 0, $height_thumb = 0, $zoom_crop = '1', $src = '', $watermark = null, $path = THUMBS, $preview = false, $args = array(), $quality = 100){
         self::createThumbNormal($width_thumb, $height_thumb, $zoom_crop, $src, $watermark, $path, $preview, $args, 100);
    }

    public function createThumbNormal($width_thumb = 0, $height_thumb = 0, $zoom_crop = '1', $src = '', $watermark = null, $path = THUMBS, $preview = false, $args = array(), $quality = 100)
    {
        global $config;
        $webp = false;
        $t = 3600 * 24 * 30;
    ...
    /* Get Product Item */
    public function GetProducts( $arr = array(), $class = '' ) {
    ...
    public function createSitemap($com = '', $type = '', $field = '', $table = '', $time = '', $changefreq = '', $priority = '', $lang = 'vi', $orderby = '', $menu = true)
    {
        ...
        if (!empty($type) && !in_array($table, ['photo', 'static'])) {
            ...
            $sitemap = \Illuminate\Support\Facades\DB::table("table_". $table)
                ->where("type", "=", $type)
                ->get();
        }
        if ($menu == true && $field == 'id') {
            $urlSm = config("app.url") . $com;
            echo '<url>';
            echo '<loc>' . $urlSm . '</loc>';
        ...
        if (!empty($sitemap)) {
            foreach ($sitemap as $value) {
                if (!empty($value->slugvi)) {
                    $urlSm = config("app.url") . $value->slugvi;
                    echo '<url>';
                    echo '<loc>' . $urlSm . '</loc>';
    ...
    /* Build Schema */
    public function buildSchemaProduct($id_pro = 0, $name = '', $image = '', $description = '', $code_pro = '', $name_brand = '', $name_author = '', $url = '', $price = 0)
    {
        $str = '{';
    ...
    public function formenu($table,$type,$showh2=1)
    {
        ...
        for($i=0;$i<count($danhmuc_cap1);$i++){
            ...
            if(count($danhmuc_cap2)>0){
                $str.='<li class="parent"><a class="has-child" href="'.$danhmuc_cap1[$i][$sluglang].'">'.$start_h2.$danhmuc_cap1[$i]["ten"].$end_h2.'</a>';
            } else {
                $str.='<li><a class="has-child" href="'.$danhmuc_cap1[$i][$sluglang].'">'.$start_h2.$danhmuc_cap1[$i]["ten"].$end_h2.'</a>';
            }
            if(count($danhmuc_cap2)>0){
                $str.='<div class="ul__wrapper">';
                $str.='<ul>';
                for($j=0;$j<count($danhmuc_cap2);$j++){
                    ...
                    if(count($danhmuc_cap3)>0){
                        $str.='<li class="parent"><a class="has-child transition" href="'.$danhmuc_cap2[$j][$sluglang].'">'.$start_h2.$danhmuc_cap2[$j]["ten"].$end_h2.'</a>';
                    } else {
    ...
    /*--- Lấy baiviet ----*/
    function lay_baiviet($truyvan,$cssitem,$thumb){
        global $lang,$sluglang;
        $str ='';
        foreach($truyvan as $k => $v){
        ...
        foreach($truyvan as $k => $v){
            ...
            <div class="album">
                <a class=" text-decoration-none" data-fancybox="gallery<?=$v['id']?>" href="<?=UPLOAD_PHOTO_L.$v['photo']?>" data-caption="<?=$v['name'.$lang]?>">
                    <p class="pic-album scale-img"><img onerror="this.src='<?=$thumb?>assets/images/noimage.png';" src="<?=$thumb?><?=UPLOAD_PHOTO_L.$v['photo']?>" alt="<?=$v['name'.$lang]?>" width="350" height="350"/></p>
                    <h3 class="name-album text-split"><span><?=$v['name'.$lang]?></span></h3>
                </a>
            </div>
        <?php  }
    ...
    /*--- Lấy video ----*/
    /*--- Lấy video highlight ----*/
    function lay_video_highlight($truyvan,$thumb){
        global $lang,$sluglang;
        foreach($truyvan as $k => $v){ ?>
            <div class="video_wrapper">
                <a class="video text-decoration-none" data-fancybox="video" data-src="<?=$v['link_video']?>" title="<?=$v['name'.$lang]?>">
                    ...
                    <?php if(isset($v['photo']) && !empty($v['photo'])) { ?>
                        <p class="pic-video scale-img"><img src="<?=$thumb . UPLOAD_NEWS_L . $v['photo']?>" alt="<?=$v['name'.$lang]?>"/></p>
                    <?php } else { ?>
                        <p class="pic-video scale-img"><img onerror="this.src='<?=$thumb?>assets/images/noimage.png';" src="https://img.youtube.com/vi/<?=$this->getYoutube($v['link_video']);?>/0.jpg" alt="<?=$v['name'.$lang]?>"/></p>
                    <?php } ?>
                </a>
    ...
    {
        $factory = $this->app(\Illuminate\View\Factory::class);
    ...
    function trans($key = null, $replace = [], $locale = null)
    {
        ...
        return $this->app('translator')->get($key, $replace, $locale);
    }
